...
├── financial_ralationship.py # 상관관계 시각화(히트맵, 버블)
├── utils/
...
- **종합 인사이트:**
  - `국고채`와 `회사채` 금리는 **2022년 중후반 고점 → 이후 하락세**
  - `원달러환율`은 **2023년 후반부터 안정적 하락 → 원화 강세**
  - `코스피지수`는 **금리/환율 하락과 맞물려 급반등**  
    → 이는 시장 전반이 **위험 회피(Risk-Off)에서 위험 선호(Risk-On)로 전환**되었음을 시사함.
  - 회사채 금리는 국고채보다 항상 높으며, 이는 **신용위험 프리미엄** 때문.  
    두 금리 간 스프레드는 일정 → 기업 신용에 대한 시장의 신뢰 유지.
...
### 🧠 시사점 및 해석 방향
- 금리는 KOSPI와 일정 수준의 반비례 관계를 가질 수 있으며, 금리 정책이 주식시장과 환율에 복합적으로 작용  
- KOSPI는 단순히 환율이나 금리 하나로 설명되지 않음 → 복합적인 요인에 의해 움직이는 구조임을 재확인
